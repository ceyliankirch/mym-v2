...
function ModalSejour({ sejourData, setSejourEnEdition, isSubmitting, setIsSubmitting }) {
  ...
  const DEFAULT_FORM = [
    ...
    { id: "25", type: "info", label: "Chèque : à l'ordre de \"{{TITULAIRE}}\", à envoyer à {{TITULAIRE}} - {{ADRESSE}}. Paiement en 3 fois autorisé.\nCarte bancaire : paiement en ligne via une plateforme du Crédit Mutuel (5€ de frais). Un lien vous sera envoyé.\nVirement : IBAN {{IBAN}}, libellé au nom et prénom du participant.\nChèques ANCV : à envoyer à {{TITULAIRE}} - {{ADRESSE}}.\nEspèces : nous contacter.", required: false },
      ...
      <div style={{ background: C.white, width: "100%", maxWidth: "750px", maxHeight: "90vh", overflowY: "auto", borderRadius: "24px", padding: "32px", position: "relative", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.3)" }}>
        ...
        <form action={async (formData) => {
          ...
          <div style={{ display: tab === "infos" ? "flex" : "none", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              <label style={{ fontSize: "11px", fontWeight: 700, color: C.gray, textTransform: "uppercase" }}>Lien de paiement — Tarif standard</label>
              <input type="url" name="lienPaiementCIC" defaultValue={isEditing ? sejourData.lienPaiementCIC : ""} placeholder="https://..." style={{ padding: "12px", borderRadius: "12px", border: `1px solid ${C.lightGray}` }} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              <label style={{ fontSize: "11px", fontWeight: 700, color: C.gray, textTransform: "uppercase" }}>Lien de paiement — Tarif Habitant du Val-de-Marne</label>
                ...
                <div key={field.id} style={{ background: field.type === 'section' ? C.teal : C.white, border: `1px solid ${field.type === 'section' ? C.teal : C.lightGray}`, borderRadius: "12px", padding: "12px", display: "flex", flexDirection: "column", gap: "12px", transition: "all 0.2s" }}>
                  ...
                  {field.type === 'info' && (
                    <p style={{ fontSize: "11px", color: C.gray, margin: 0 }}>
                      Astuce : utilisez ces jetons pour insérer automatiquement des valeurs à jour — <code>{"{{ASSURANCE}}"}</code> (montant de l'assurance annulation, ci-dessus), <code>{"{{IBAN}}"}</code>, <code>{"{{TITULAIRE}}"}</code>, <code>{"{{ADRESSE}}"}</code> (coordonnées définies dans Admin / Paramètres).
                    </p>
                  )}

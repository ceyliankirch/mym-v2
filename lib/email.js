...
function virementBox({ iban, bic, titulaire, reference, montant, enAttenteChambre }) {
  if (!iban) return "";
  ...
  return `
    <div style="background:${C.arctic}; border-radius:16px; padding:20px 24px; margin: 8px 0 24px;">
      ...
      ${enAttenteChambre
        ? `<p style="margin:12px 0 0; font-size:13px; color:#9a3412; line-height:1.6;">⏳ Vous êtes actuellement en liste d'attente d'un binôme pour la chambre double : merci de <strong>n'effectuer ce virement qu'une fois</strong> qu'un binôme aura été trouvé, ou que vous aurez été basculé(e) sur le tarif chambre simple. Vous serez informé(e) par email dans tous les cas.</p>`
        : montant
        ? `<p style="margin:12px 0 0; font-size:13px; color:#41545c; line-height:1.6;">Merci d'indiquer la référence ci-dessus dans le libellé de votre virement de <strong>${montant}</strong>.</p>`
        : ""}
    </div>
  `;
}
...
export async function sendInscriptionConfirmationEmail({
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ...
      ${virement?.iban ? virementBox({ ...virement, reference: nomComplet || prenomEnfant, montant, enAttenteChambre: enListeAttenteChambre }) : ""}

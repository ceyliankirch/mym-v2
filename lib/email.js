              ...
              <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%;">
                ...
                <tr>
                  <td align="center" style="padding: 24px 16px 0;">
                    <p style="margin:0; font-size:12px; color:#8aaa; line-height:1.6;">
                      Make Your Moment — Association loi 1901<br />
                      <a href="${SITE_URL}" style="color:#8aaa; text-decoration:underline;">make-your-moment.com</a>
                    </p>
                  </td>
                </tr>
              </table>
...
// 🏦 Bloc "Instructions de paiement par virement", affiché quand la famille a choisi
// ce moyen de paiement et que l'IBAN de l'association est configuré (Admin → Paramètres)
function virementBox({ iban, bic, titulaire, reference, montant }) {
  if (!iban) return "";
  const ligne = (label, value) =>
    value
      ? `<tr><td style="padding: 5px 0; font-size:14px; color:${C.teal};"><strong>${label} :</strong> ${value}</td></tr>`
      : "";

  return `
    <div style="background:${C.arctic}; border-radius:16px; padding:20px 24px; margin: 8px 0 24px;">
      <p style="margin:0 0 8px; font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:1px; color:${C.teal};">Instructions de paiement par virement</p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        ${ligne("Titulaire du compte", titulaire)}
        ${ligne("IBAN", iban)}
        ${ligne("BIC", bic)}
        ${ligne("Référence à indiquer", reference)}
      </table>
      ${montant ? `<p style="margin:12px 0 0; font-size:13px; color:#41545c; line-height:1.6;">Merci d'indiquer la référence ci-dessus dans le libellé de votre virement de <strong>${montant}</strong>.</p>` : ""}
    </div>
  `;
}

// 💶 Modalités de paiement selon le moyen choisi par la famille (email de pré-inscription)
function modalitesPaiementBox({ moyenPaiement, virement, courrier, reference, montant }) {
  const moyen = String(moyenPaiement || "").trim().toLowerCase();
  const destinataire = [courrier?.titulaire, courrier?.adresse].filter(Boolean).join(" - ");
  let titre = "";
  let corps = "";

  if (moyen.includes("virement")) {
    return virementBox({ ...(virement || {}), reference, montant });
  } else if (moyen.includes("carte")) {
    titre = "Paiement par carte bancaire";
    corps = "Le paiement s'effectue en ligne via la plateforme sécurisée du Crédit Mutuel (5 € de frais bancaires inclus dans le montant ci-dessus). Utilisez le bouton de paiement ci-dessous.";
  } else if (moyen.includes("ancv") || moyen.includes("vacances")) {
    titre = "Paiement par chèques-vacances (ANCV)";
    corps = `Merci d'envoyer vos chèques-vacances${destinataire ? ` à : <strong>${destinataire}</strong>` : " à l'association"}, en précisant le nom et le prénom du participant.`;
  } else if (moyen.includes("chèque") || moyen.includes("cheque")) {
    titre = "Paiement par chèque";
    corps = `Merci d'établir votre chèque à l'ordre de <strong>${courrier?.titulaire || "l'association"}</strong> et de l'envoyer${courrier?.adresse ? ` à : <strong>${destinataire}</strong>` : ""}. Le paiement en 3 fois est autorisé.`;
  } else if (moyen.includes("espèce") || moyen.includes("espece") || moyen.includes("liquide")) {
    titre = "Paiement en espèces";
    corps = "Le règlement en espèces se fait directement auprès de notre équipe : merci de nous contacter pour convenir d'un rendez-vous de remise.";
  } else {
    return "";
  }
  ...
  return `
    <div style="background:${C.arctic}; border-radius:16px; padding:20px 24px; margin: 8px 0 24px;">
      <p style="margin:0 0 8px; font-size:12px; font-weight:800; text-transform:uppercase; letter-spacing:1px; color:${C.teal};">${titre}</p>
      <p style="margin:0; font-size:14px; color:#41545c; line-height:1.6;">${corps}</p>
    </div>
  `;
}
...
async function send({ to, subject, html, attachments, bcc }) {
  ...
  const bccList = bcc ? (Array.isArray(bcc) ? bcc : [bcc]).filter(Boolean).map((e) => ({ email: e })) : null;
  try {
...
// 🩺 Fiche sanitaire de liaison (document CERFA fourni par l'association).
// Le titre porte l'année du séjour (ex. "ANNEE 2026") : on recouvre la mention d'origine
// et on la réécrit à chaque envoi à partir de la date de début du séjour.
async function ficheSanitaireAttachment(dateDebut) {
  try {
    const filePath = path.join(process.cwd(), "public", "fiche-sanitaire-de-liaison.pdf");
    const annee = (dateDebut ? new Date(dateDebut) : new Date()).getFullYear();
    const pdf = await PDFDocument.load(fs.readFileSync(filePath));
    const page = pdf.getPage(0);
    const font = await pdf.embedFont(StandardFonts.HelveticaBold);
    page.drawRectangle({ x: 50, y: 658, width: 230, height: 24, color: rgb(1, 1, 1) });
    page.drawText(`ANNEE ${annee}`, { x: 54.4, y: 665, size: 18, font, color: rgb(0, 0, 0) });
    const buffer = Buffer.from(await pdf.save());
    return pdfAttachment(buffer, "fiche-sanitaire-de-liaison.pdf");
  } catch (e) {
    console.error("Erreur génération fiche sanitaire de liaison", e);
    return [];
  }
}
...
// 💳 Pré-inscription envoyée à la soumission, avec les modalités du moyen de paiement choisi.
// Par carte : bouton vers le bon lien (standard ou Val-de-Marne). La famille doit payer avant
...
export async function sendPreInscriptionEmail({
  ...
  pdfBuffer,
  moyenPaiement,
  virement, // { iban, bic, titulaire } — pour le virement
  courrier, // { titulaire, adresse } — destinataire des chèques / chèques-vacances
  nomComplet,
}) {
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ...
      ${montant ? paragraph(`Pour rappel, le montant de votre inscription est de <strong>${montant}</strong>. Votre inscription sera validée une fois le paiement effectué.`) : ""}
      ${modalitesPaiementBox({ moyenPaiement, virement, courrier, reference: nomComplet || prenomEnfant, montant })}
      ${lienPaiement || documentsManquants?.length ? paragraph("Pour finaliser le dossier, il ne reste plus qu'à :") : ""}
      ${prochainesEtapes({
        lienPaiementCIC: lienPaiement,
        documents: documentsManquants,
      })}
      ${montant && lienPaiement ? footNote("Si vous avez déjà réglé le séjour, vous pouvez ignorer le bouton de paiement ci-dessus.") : ""}
    `,
  });
...
export async function sendInscriptionConfirmationEmail({
  ...
  documentsRequis,
  montantARegler,
  virement, // { iban, bic, titulaire } — renseigné quand la famille a choisi de payer par virement
  enListeAttenteChambre, // séniors : en attente d'un binôme pour la chambre double
  inclureFicheSanitaire = true, // configurable par séjour (Admin → édition du séjour)
}) {
  if (!to) return { success: false, error: "No email address" };
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ...
      ${enListeAttenteChambre
        ? paragraph("Vous avez choisi la chambre double en recherche d'un binôme : le paiement du séjour vous sera demandé plus tard, une fois la répartition des chambres finalisée.")
        : montant ? paragraph(`Pour rappel, le montant à régler pour ce séjour est de <strong>${montant}</strong>.`) : ""}
      ${!enListeAttenteChambre && virement?.iban ? virementBox({ ...virement, reference: nomComplet || prenomEnfant, montant }) : ""}
      ${inclureFicheSanitaire ? paragraph("Vous trouverez ci-joint la fiche sanitaire de liaison : merci de la remplir et de l'uploader dans votre espace famille.") : ""}
  ...
  return send({
    ...
    html,
    attachments: inclureFicheSanitaire ? await ficheSanitaireAttachment(dateDebut) : [],
  });
}
...
// 📝 Demande à la famille de re-compléter le formulaire d'inscription (prétexte : incident technique)
export async function sendDemandeReinfoEmail({ to, prenomEnfant, sejourTitre }) {
  if (!to) return { success: false, error: "No email address" };

  const pour = prenomEnfant ? ` de <strong>${prenomEnfant}</strong>` : "";
  const html = emailShell({
    title: "Informations à recompléter",
    emoji: "📝",
    bodyHtml: `
      ${paragraph("Bonjour,")}
      ${paragraph(`Suite à un incident technique survenu sur nos serveurs, les informations que vous aviez renseignées pour l'inscription${pour} au séjour <strong>${sejourTitre}</strong> n'ont pas pu être enregistrées correctement.`)}
      ${paragraph("Merci de bien vouloir vous reconnecter à votre espace famille et de compléter à nouveau le formulaire d'inscription — cela ne prend que quelques minutes.")}
      ${ctaButton("Compléter mes informations →", `${SITE_URL}/espace-famille`)}
      ${footNote("Toutes nos excuses pour la gêne occasionnée. Notre équipe reste à votre disposition pour toute question.")}
    `,
  });

  return send({
    to,
    bcc: "contact@make-your-moment.com",
    subject: `Informations à recompléter — ${sejourTitre}`,
    html,
  });
}
...
// ✅ Confirmation à la famille que ses informations d'inscription ont été mises à jour
export async function sendInfosMisesAJourEmail({ to, prenomEnfant, sejourTitre, reponses }) {
  if (!to) return { success: false, error: "No email address" };

  const pour = prenomEnfant ? ` de <strong>${prenomEnfant}</strong>` : "";
  const lignes = reponses && typeof reponses === "object"
    ? Object.entries(reponses)
        .map(([k, v]) => `
          <tr>
            <td style="padding:8px 0; font-size:13px; color:#8aaa; font-weight:700; vertical-align:top; width:45%;">${k}</td>
            <td style="padding:8px 0; font-size:13px; color:${C.teal}; font-weight:700;">${v === true ? "Oui" : v === false ? "Non" : String(v)}</td>
          </tr>`)
        .join("")
    : "";
  const recap = lignes
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.arctic}; border-radius:16px; padding:8px 24px; margin:8px 0 24px;">${lignes}</table>`
    : "";
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ${paragraph("Bonjour,")}
      ${paragraph(`Vos informations pour l'inscription${pour} au séjour <strong>${sejourTitre}</strong> ont bien été mises à jour. Voici le récapitulatif :`)}
      ${recap}
      ${footNote("Si une information est incorrecte, connectez-vous à votre espace famille pour la corriger, ou contactez-nous.")}
    `,
  });
  ...
  return send({
    to,
    bcc: "contact@make-your-moment.com",
    subject: `Informations mises à jour — ${sejourTitre}`,
    html,
  });
}
...
export async function sendWelcomeEmail({ to, prenom }) {
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ...
      ${paragraph("Votre compte a bien été créé. Vous pouvez dès maintenant ajouter vos proches et les inscrire à nos séjours et sorties.")}
      ${ctaButton("Accéder à mon espace famille →", `${SITE_URL}/espace-famille`)}

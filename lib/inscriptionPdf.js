...
export function generateInscriptionPdf({ enfant, client, sejour, documentsRequis, montantTotal }) {
  return new Promise((resolve, reject) => {
    ...
    // Montant réellement choisi par la famille (tarif Val-de-Marne, options...) ; à défaut, tarif de base
    const tarif = montantTotal ?? sejour?.prix;
    row(doc, "Tarif", tarif ? `${Number(tarif).toFixed(2).replace(".", ",")} €` : "Non renseigné");

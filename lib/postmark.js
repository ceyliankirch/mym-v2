...
import { genererTokenDesabonnement } from "./unsubscribeToken.js";
...
const FROM_ADDRESS = "Make Your Moment <noreply@make-your-moment.com>";
...
/* ─── NEWSLETTER ──────────────────────────────────────────────────
   Le HTML des campagnes est déjà un document complet (édité par l'admin),
   on ne le fait donc pas passer par emailShell(). L'envoi de masse utilise
   un message stream dédié ("broadcast" par défaut, configurable via
   POSTMARK_BROADCAST_STREAM) : Postmark exige un stream Broadcast approuvé
   séparément du stream transactionnel "outbound" pour ce type d'envoi. */

// 📧 Envoi d'un email de test (sur le stream transactionnel, toujours actif)
export async function sendNewsletterTest({ to, sujet, htmlContent }) {
  return send({ to, subject: sujet, html: htmlContent });
}

// 🔗 Lien de désabonnement personnalisé, ajouté en pied de chaque envoi
function lienDesabonnement(email) {
  const token = genererTokenDesabonnement(email);
  return `${SITE_URL}/desabonnement?email=${encodeURIComponent(email)}&token=${token}`;
}

function ajouterPiedDesabonnement(htmlContent, email) {
  const lien = lienDesabonnement(email);
  const footer = `
      ...
      <tr>
        <td align="center" style="padding: 16px;">
          <p style="margin:0; font-size:12px; color:#8aaa; line-height:1.6;">
            ...
            <a href="${lien}" style="color:#8aaa; text-decoration:underline;">Se désabonner</a>
          </p>
        </td>
      </tr>
    </table>
...
// 📬 Envoi en masse d'une campagne à une liste de destinataires
export async function sendNewsletterBatch({ recipients, sujet, htmlContent }) {
  const client = getPostmarkClient();
  if (!client) {
    return { success: false, error: "POSTMARK_API_TOKEN not configured" };
  }
  ...
  const CHUNK_SIZE = 500; // limite Postmark par appel sendEmailBatch
    ...
    for (let i = 0; i < recipients.length; i += CHUNK_SIZE) {
      ...
      const messages = chunk.map((email) => {
        ...
        return {
          ...
          To: email,
          Subject: sujet,
          HtmlBody: ajouterPiedDesabonnement(htmlContent, email),

...
const SITE_URL = process.env.NEXT_PUBLIC_APP_URL || "https://make-your-moment.com";
...
export async function sendInscriptionReceivedEmail({ to, prenomEnfant, sejourTitre, pdfBuffer }) {
...
export async function sendDocumentsRequestEmail({ to, prenomEnfant, sejourTitre, documentsManquants, pdfBuffer }) {
  if (!to) return { success: false, error: "No email address" };
  ...
  const html = emailShell({
    ...
    bodyHtml: `
      ...
      ${paragraph("Pour finaliser le dossier, il ne reste plus qu'à nous transmettre les documents suivants :")}
      ${documentsBox({ title: "Documents à fournir", documents: documentsManquants })}
...
export async function sendInscriptionConfirmationEmail({
  ...
  const etapes = `
    ...
    ${lienPaiementCIC ? `
      ${paragraph("1. Régler le séjour en ligne, en toute sécurité :")}
      ${ctaButton("Procéder au paiement →", lienPaiementCIC, C.yellow, C.teal)}
    ` : ""}
    ${documentsRequis && documentsRequis.length > 0 ? `
      ${paragraph(`${lienPaiementCIC ? "2." : "1."} Nous transmettre les documents suivants :`)}
      ${documentsBox({ title: "Documents à fournir", documents: documentsRequis })}
    ` : ""}
  `;
  ...
  return send({ to, subject: `Inscription confirmée pour ${prenomEnfant} - ${sejourTitre}`, html });
}
    ...
    bodyHtml: `
      ...
      <div style="background:${C.arctic}; border-radius:16px; padding:20px 24px; margin: 8px 0 24px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
          ${infoRow("Enfant", `${prenomEnfant} ${nomEnfant}`)}
          ${infoRow("Représentant légal", `${clientPrenom || ""} ${clientNom || ""}`)}
          ${infoRow("Email", clientEmail)}
          ${infoRow("Téléphone", clientTelephone)}
        </table>
      </div>
